...
$flight = $conn->query("SELECT * FROM flight where status = 1");
	...
	<form id="manage_schedule">
		<div class="col-md-12">
			<div class="form-group mb-2">
				<label for="flight_id" class="control-label">Flight Name</label>
				...
				<select name="flight_id" id="flight_id" class="form-control" required>
					<option value="" <?php echo isset($meta['flight_id']) && $meta['flight_id'] > 0 ? '' : 'selected'  ?> disabled="">Select Here</option>
					<?php while($row = $flight->fetch_assoc()){ ?>
						<option value="<?php echo $row['id'] ?>" <?php echo isset($meta['flight_id']) && $meta['flight_id'] == $row['id'] ? 'selected' : ''  ?>><?php echo $row['flight_number'] . ' | '.$row['name'] ?></option>
					<?php } ?>
				</select>
			</div>
			<div class="form-group mb-2">
