...
$qry = $conn->query("SELECT s.*,concat(b.flight_number,' | ',b.name) as flight FROM schedule_list s inner join flight b on b.id = s.flight_id where s.status = 1 ".$where." order by date(s.departure_time) asc");
$data = array();
